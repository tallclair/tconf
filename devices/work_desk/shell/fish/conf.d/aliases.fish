...
abbr -a kind-test-e2e 'ginkgo --focus="$FOCUS" ./test/e2e -- -context kind-kind -num-nodes 2'
...
abbr -a kind-rebuild-e2e-cluster 'kind delete cluster && kind build node-image && kind create cluster --config $HOME/src/github.com/tallclair/k8s-devel/config/kind-e2e-config.yaml --image kindest/node:latest'
